re_mits.qp.est <- function(gen,phe,loci){
  ...
  st <- table(nsnp)
  h0 <- try(mits.qp.h0(ny),TRUE)
  if (class(h0)=="try-error"){
    h0<-NA
  }
  par1 <- h0[-1]
  h1 <- try(mits.qp.h1(par1,ny,nsnp),TRUE)
...
mits.qp.est <- function(gen,phe,interval=c(1,10)){
...
mits.qp.h0<-function(ny){
  
  init.par<-c(1.06,0.067,-0.320,0,0)
  covar.par <- c(0.5,0.1)
...
mits.qp.h1<- function(par1,ny,nsnp){
  ...
  for(i in 1:length(snp.type)){
    ...
    s.m <- optim(par1[3:7],s.mle2,s.y=SNP.p,s.l=light,
