re_logi.etr.est <- function(gen,phe,loci){
  ...
  st <- table(nsnp)
  h0 <- try(logi.etr.h0(ny),TRUE)
  if (class(h0)=="try-error"){
    h0<-NA
  }
  par1 <- h0[-1]
  h1 <- try(logi.etr.h1(par1,ny,nsnp),TRUE)
...
logi.etr.est <- function(gen,phe,interval=c(1,10)){
...
logi.etr.h0<-function(ny){
  
  init.par<-c(1.038,3109,1.339,0.009336,1.328)
  covar.par <- c(0.5,0.1)
...
logi.etr.h1<- function(par1,ny,nsnp){
  ...
  for(i in 1:length(snp.type)){
    ...
    s.m <- optim(par1[3:7],s.mle1,s.y=SNP.p,s.l=light,
